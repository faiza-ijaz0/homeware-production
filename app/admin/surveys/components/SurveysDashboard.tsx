...
interface Survey {
  ...
  // This matches exactly what's in Firebase
  selectedClient?: {
    id: string
    name: string
    company: string
...
export default function SurveysDashboard({
  ...
  useEffect(() => {
    ...
    surveys.forEach((survey, index) => {
      console.log(`📊 Survey ${index}: ${survey.title}`, {
        ...
        hasSelectedClient: !!survey.selectedClient,
  ...
  const filteredSurveys = useMemo(() => {
    return surveys.filter(survey => {
      // Get client name from selectedClient ONLY - no fallbacks
  ...
  // Helper function to get client display info - prioritize selectedClient
  const getClientInfo = (survey: Survey) => {
    // Debug: Log each survey's client data
    console.log(`🔍 Getting client info for survey: ${survey.title}`, {
      hasSelectedClient: !!survey.selectedClient,
      selectedClient: survey.selectedClient,
      clientName: survey.clientName,
      company: survey.company
    ...
    // ✅ FIXED: Directly use selectedClient if available
    ...
    // Only use fallbacks if selectedClient doesn't exist (old data)
    return {
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-200">
                {filteredSurveys.map((survey) => {
                  ...
                  console.log(`🎨 Rendering survey ${survey.title}:`, {
                    clientInfo,
                    rawSelectedClient: survey.selectedClient
                  })
                      ...
                      <td className="px-4 py-3">
                        <div>
                          <div className="flex items-center gap-1">
                            {/* ✅ This will now show "Mr Hammad" for your data */}
                            <p className="font-medium text-black">{clientInfo.name}</p>
                            {clientInfo.type && (
                              <span className={`text-[8px] px-1 py-0.5 rounded ${
                                ...
                                {clientInfo.type.toUpperCase()}
                              </span>
                            )}
                          </div>
                          {/* ✅ This will show "NASAB BUSINESS CENTER" for your data */}
                          <p className="text-xs text-gray-500">{clientInfo.company}</p>
                          <p className="text-xs text-gray-400">{clientInfo.serviceType}</p>
                        </div>
                      </td>
